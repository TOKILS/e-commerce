...
describe("test v2 routes /api/v2/Category", () => {
  ...
  it("test v3 getAll ", async () => {
    const resToken = await req.post("/signin").auth("admin", "password");
    const token = resToken.body.token;

    const res = await req
      .get("/api/v3/reviews/1")
      .set("Authorization", `Bearer ${token}`);
    expect(res.status).toBe(200);
  });
  it("test v3 getAll ", async () => {
  ...
  it("test v3 getAll ", async () => {
    ...
    const res = await req
      .get("/api/v3/wishlistProductsInfo/1")
      .set("Authorization", `Bearer ${token}`);
    expect(res.status).toBe(200);
  });
  it("test v3 getAll ", async () => {
  ...
  it("test v3 getAll ", async () => {
    ...
    const res = await req
      .get("/api/v3/orderProductsInfo/1")
      .set("Authorization", `Bearer ${token}`);
    expect(res.status).toBe(200);
  });
  it("test v3 getAll ", async () => {
